  ...
  return (
    <section id="events" className="py-24 bg-brand-orange-light/5 text-foreground">
      <div className="container mx-auto px-4">
        <h2 className="text-4xl font-heading font-bold mb-12 text-center tracking-tight text-brand-teal">
          開催予定のイベント
        </h2>
    ...
    <article className="bg-background rounded-2xl overflow-hidden shadow-lg flex flex-col group hover:-translate-y-2 transition-transform duration-300 border border-foreground/5">
      ...
      <div className="p-6 flex flex-col flex-grow gap-4">
        <div className="flex flex-col gap-1 text-sm font-bold text-brand-orange-dark">
          ...
          <span className="opacity-80 text-foreground">
            {event.location || 'オンライン / 未定'}
          </span>
        </div>
        <h3 className="font-bold font-heading text-xl leading-tight text-balance group-hover:text-brand-orange-dark transition-colors">
          {event.title}
        </h3>
        ...
        <Link
          ...
          className="mt-4 text-brand-teal font-bold hover:opacity-70 transition-opacity self-start inline-flex items-center gap-1"
        >
