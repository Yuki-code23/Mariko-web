...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  const imageUrl = project.img?.url || 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80';
  ...
  return (
    ...
    >
      <div className="aspect-square overflow-hidden relative">
        <div className={`absolute inset-0 ${project.color || 'bg-brand-pink'} mix-blend-multiply opacity-0 group-hover:opacity-40 transition-opacity z-10`} />
        <Image src={imageUrl} alt={project.title} fill className="object-cover group-hover:scale-110 transition-transform duration-700" sizes="(max-width: 768px) 100vw, 25vw" />
      </div>
      <div className="p-6 flex flex-col flex-grow gap-3">
        ...
        <Link href="#" onClick={(e) => { e.preventDefault(); alert('詳細ページは現在準備中です。'); }} className={`mt-4 inline-flex items-center text-sm font-bold ${project.colorText || 'text-brand-pink'} hover:opacity-70 transition-opacity`}>
          詳しく見る &rarr;
        </Link>
      </div>
    </motion.div>
